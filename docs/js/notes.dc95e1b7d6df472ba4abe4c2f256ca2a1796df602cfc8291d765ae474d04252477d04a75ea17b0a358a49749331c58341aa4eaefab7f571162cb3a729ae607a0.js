(() => {
  ...
  (function() {
    ...
    function loadIndex() {
      ...
      if (!pending) {
        pending = fetch(root.getAttribute("data-index-url")).then(function(res) {
          return res.json();
        }).then(function(data) {
          index = data || [];
          return index;
    ...
    function render(items) {
      ...
      items.forEach(function(item) {
        var tags = (item.g || []).map(function(tag) {
          return '<button type="button" class="notes-search-tag" data-tag="' + escapeHtml(tag) + '">#' + escapeHtml(tag) + "</button>";
        }).join("");
        html += '<a class="notes-search-item" href="' + escapeHtml(item.u) + '"><span class="notes-search-title">' + escapeHtml(item.t) + "</span>" + (item.c ? '<span class="notes-search-chapter">' + escapeHtml(item.c) + "</span>" : "") + '<span class="notes-search-tags">' + tags + "</span></a>";
      });
